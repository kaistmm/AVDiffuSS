    ...
    def reverse(oself, score_model, probability_flow=False, diffusion_power_gradient=None):
        ...
        class RSDE(oself.__class__):
            ...
            def discretize(self, x, t, *args, **kwargs):
                ...
                if "conditioning" in kwargs.keys() and kwargs["conditioning"] is not None:
                    score_res = score_model(x, t, score_conditioning=kwargs["conditioning"], sde_input=args[0], **kwargs) # forward_score()
                    if len(score_res)==2:
                        score_res, attn_scores = score_res
                    else:
                        attn_scores = None
                    rev_f = f - G ** 2 * score_res * (0.5 if self.probability_flow else 1.)
                else:
                    attn_scores = None
                    rev_f = f - G ** 2 * score_model(x, t, *args) * (0.5 if self.probability_flow else 1.)
                rev_G = torch.zeros_like(G) if self.probability_flow else G
                if attn_scores is not None:
                    return rev_f, rev_G, attn_scores
                else: # default
                    return rev_f, rev_G
