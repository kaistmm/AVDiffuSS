...
class StochasticRegenerationModel(pl.LightningModule):
    def __init__(self,
        ...
        **kwargs #,loss_type_transformer: str = 'mse'
    ):
        """
        ...
        #self.loss_type_transformer =loss_type_transformer
    ...
    def _loss(self, err, y_denoised, x,y_trans):
        loss_score = self.loss_fn_score(err) if self.loss_type_score != "none" else None #mse
    ...
    def forward_score(self, x, t, score_conditioning, sde_input, context=None, **kwargs):
        dnn_input = torch.cat([x] + score_conditioning, dim=1) #b,n_input*d,f,t
        score = self.score_net(dnn_input, time_cond = t,context= context)
    ...
    def _step(self, batch, batch_idx):
        ...
        if self.condition == "noisy":
            ...
            score_conditioning = [y_denoised] #[y_denoised]
        elif self.condition == "both":
            score_conditioning = [y, y_denoised] #[y, y_denoised]
        else:
            raise NotImplementedError(f"Don't know the conditioning you have wished for: {self.condition}")
        score = self.forward_score(perturbed_data, t, score_conditioning, sde_input, visualFeatures)
        err = score * sigmas + z

        loss, loss_score, loss_denoiser = self._loss(err, y_denoised, x, y_trans)
    ...
    def validation_step(self, batch, batch_idx, discriminative=False, sr=16000):
        ...
        if batch_idx == 0 and self.num_eval_files != 0:
            ...
            if self.current_epoch %2 ==0 and self.current_epoch !=0:
                num_eval_files =1000
            pesq_est, si_sdr_est, estoi_est, spec, audio = evaluate_model(self, num_eval_files, spec=not self.current_epoch%VIS_EPOCHS, audio=not self.current_epoch%VIS_EPOCHS, discriminative=discriminative)
    ...
    def enhance(self, y,context, sampler_type="pc", predictor="reverse_diffusion",
        ...
        **kwargs
        ...
        with torch.no_grad():
            ...
            if self.score_net is not None and not denoiser_only:
                ...
                # Reverse process
                if sampler_type == "pc":
                    sampler = self.get_pc_sampler(predictor, corrector,y= Y_denoised, N=N,
                ...
                if "crossatt" in self.backbone_score:
                    sample, nfe = sampler(context)
                else:
                    sample, nfe = sampler()
            else:
                sample = Y_denoised
